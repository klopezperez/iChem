...
class BitBirch():
    ...
    def fit(self, X, indices = None):
        ...
        return self._fit(X, indices = None)

    def _fit(self, X, indices = None):
        threshold = self.threshold
        branching_factor = self.branching_factor
        ...
        for idx, sample in zip(iter_func(X), indices):
            set_bits = np.sum(sample)
            subcluster = _BFSubcluster(linear_sum=sample, mol_indices = [idx])
    ...
    def fit_BFs(self, X):
        ...
        for sample in iter(X):
            ...
            split = self.root_.insert_bf_subcluster(cluster, set_bits)
